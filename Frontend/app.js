...
function startStatusPolling(scanId) {
  ...
  let nextPollAt = Date.now() + pollIntervalMs;
  ...
  if (summary.sections && summary.sections.length === 0 && summary.message) {
    container.innerHTML = `
      <div class="summary-section">
        ...
        <div class="summary-timestamp">${section && section.timestamp ? 'Completed: ' + formatDate(section.timestamp) : 'Report generated: ' + formatDate(createdAt)}</div>
      </div>
    `;
    const scanFilter = document.getElementById("scanSummaryFilter");
    ...
    switch (section.type) {
      ...
      case "combined_summary":
        ...
        contentHtml = `
          ...
          <div class="summary-timestamp">${actionableIntelligence && actionableIntelligence.timestamp ? 'Completed: ' + formatDate(actionableIntelligence.timestamp) : (section && section.timestamp ? 'Completed: ' + formatDate(section.timestamp) : 'Report generated: ' + formatDate(createdAt))}</div>
        `;
        break;
    }

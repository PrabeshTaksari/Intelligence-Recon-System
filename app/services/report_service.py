...
class ReportService:
    ...
    async def generate_html_report(
        ...
        if scan_id in _REPORT_HTML_CACHE and not for_pdf:
            return _REPORT_HTML_CACHE[scan_id]
    ...
    def _build_html_report(
        ...
        tools_to_run = json.loads(scan.ai_tools_to_run) if scan.ai_tools_to_run else []
        user_tools = json.loads(scan.user_selected_tools) if scan.user_selected_tools else []
        tools_used = ", ".join(tools_to_run or user_tools or [tr.tool_name for tr in tool_runs])
        owasp_name = next(
            (c["name"] for c in settings.OWASP_CATEGORIES if c["id"] == scan.owasp_category),
            scan.owasp_category,
        )
        ...
        for severity in ["critical", "high", "medium", "low", "info"]:
            ...
            sev_badge = {"critical": "danger", "high": "danger", "medium": "warning", "low": "info", "info": "secondary"}.get(severity, "secondary")
            for idx, f in enumerate(sev_findings, 1):
                ...
                owasp_esc = html.escape(f.owasp_category or "-")
                findings_html += f"""
        ...
        # Risk level for summary (from severity counts)
        ...
        if c or h:
            risk_level = "CRITICAL" if c else "HIGH"
        elif m:
            risk_level = "MEDIUM"
        elif low:
            risk_level = "LOW"
        ...
        try:
            ...
            if reason:
                ai_reasoning_esc = ReportService._escape(reason)
            else:
